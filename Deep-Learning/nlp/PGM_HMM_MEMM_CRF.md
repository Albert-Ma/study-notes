...
隐藏的马尔可夫链随机生成的状态序列，称为状态序列（state sequence),每一个称为状态变量${y_{1},y_{2},...,y_{n}} $，通常假定状态变量是隐藏的、不可被观测的，因此状态变量
也称为“隐变量”;每个状态变量生成一个观测变量${x_{1},x_{2},...,x_{n}}$，而由此产生的观测的随机序列，称为观测序列（observation sequence)。观测变量的取值仅依赖于状态变量，
即$x_{i}$由$y_{i}$确定，与其他状态变量和观测变量无关；同时，i时刻的状态$y_{i}$仅依赖前一时刻的状态，与其他状态无关，这就是所谓的“马尔可夫链”：
系统下一时刻的状态仅有当前状态决定，不依赖于以往任何状态。基于这种依赖关系，所有变量的联合概率分布如下图：

![hmm1](images/hmm.png)

上图中的x就是观测变量。除了观测变量X和隐变量y，HMM 还需要确定状态转移概率A、输出观测概率B、初始状态概率$\pi$。HMM模型需要学习这五组参数(X,Y,A,B,$\pi$).

举例来说：应用到词性标注中，x代表词语，是可以观察到的。y代表我们要预测的词性（一个词可能对应多个词性）是隐含状态。应用到分词中，x代表词语，
是可以观察的,y代表我们的标签（B,E这些标签，代表一个词语的开始，或者中间等等）。应用到命名实体识别中，x代表词语，是可以观察的。y代表我们的标
签（标签代表着地点词，时间词这些）.其他的A,B,$\pi$，这些概率分布信息（上帝信息）是需要学习过程中确定的。

HMM的三个基本问题：概率计算、解码和学习问题：

1. 概率计算问题。给定HMM模型λ（A,B,$\pi$）和观测序列x,如何计算给定模型下状态序列y出现的概率 P(Y|λ)？前向、后向算法
2. 解码问题。给定一个HMM λ 和一个观测序列 x ，如何最大化 P(Q|X)，求出“最有可能”的状态序列 Q ？维特比算法
3. 学习问题（learning）。给定若干观测序列 X（对应的状态序列 Q 可能已知也可能未知），如何训练出模型参数 λ ，使得 P(X|λ) 最大化？——EM算法
...
可以看出上图就是局部马尔科夫性，在$Y_{w}$条件下，$Y_{v}$和$Y_{o}$条件独立。明白
